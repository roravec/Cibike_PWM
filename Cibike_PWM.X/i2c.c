...
//    PORTAbits.RA3 = 1;      // SDA
    ...
    I2C2BRG = (uint32_t)BRG;     // Set baud rate
...
{ 
    while((I2C2CON & 0x1F) /*|| (I2C2STAT & 0x04)*/); // Acknowledge sequence not in progress
...
{
    if (wait_ack) while (I2C2STATbits.ACKSTAT == 1);
}
